...
- **The `runner` type is the only supported `repo_type`.** Adding a framework or other type to this template is out of scope; framework-shape repos derive from a different reference (e.g. [`NWarila/terraform-framework-example`](https://github.com/NWarila/terraform-framework-example) for the do-nothing reference).
