...
- **Auto-merge never reads PR-controlled content under `pull_request_target`.** The primary control is the reusable's design: it separates read-only author authorization from the write-token merge job, trusts only a closed bot list, and never checks out PR content. As defense-in-depth, the OPA `repo_hygiene` policy denies a denylist of known checkout/PR-head reference patterns in `pull_request_target` workflows and the auto-merge reusable; this substring denylist is not an exhaustive guarantee, so the authorize/merge split remains the load-bearing safeguard.
...
- Runner consumers do not own `verify.py`, OPA policy, contract fixtures, or
  ...
  `.github/workflows/reusable-*.yaml`. `pull_request_target` safety is checked
  by the org `repo_hygiene` policy (run via the `repo-hygiene.yaml` caller as an
  advisory CI check — no `required_status_checks` rule makes it merge-blocking),
