...
`Live S3 Backend Smoke` must also succeed on `main` or via manual dispatch. It
is intentionally post-merge because it assumes the AWS OIDC role and writes the
template's real S3 state object.
